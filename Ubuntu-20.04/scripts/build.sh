...
function build_iso() {
    ...
    rm -rf image
    mkdir -p image/{casper,isolinux,install,tmpiso}

    # Get .boot files needed for ipxe boots
    sudo wget -O image/ubuntu-live.iso https://releases.ubuntu.com/20.04.3/ubuntu-20.04.3-live-server-amd64.iso 
    sudo mount -o loop image/ubuntu-live.iso image/tmpiso
    sudo cp -R image/tmpiso/.disk image/ 
    sudo umount image/tmpiso
    sudo rm -rf image/tmpiso
    sudo rm -rf image/ubuntu-live.iso
